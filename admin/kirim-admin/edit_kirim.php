<?php
    ...
    if(isset($_GET['kode'])){
        $sql_cek = "SELECT * FROM 
		tb_kirim WHERE id_transaksi='".$_GET['kode']."'";
...
<div class="card card-success">
	...
	<form action="" method="post" enctype="multipart/form-data">
		<div class="card-body">
          ...
          <div class="form-group row">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-6">
                   <input name="rek" type="text" required class="form-control" id="harga_jual" autocomplete="off" value="<?php echo $data['penerima']; ?>" readonly>
                </div>
              </div>
               <div class="form-group row">
                <label class="col-sm-2 control-label">Alamat</label>
                <div class="col-sm-6">
                   <input name="rek" type="text" required class="form-control" id="harga_jual" autocomplete="off" value="<?php echo $data['alamat']; ?>" readonly>
                </div>
              </div>
              <div class="form-group row">
		...
		<div class="card-footer">
			...
			<a href="?page=antar-admin" title="Kembali" class="btn btn-secondary">Batal</a>
		</div>
	</form>
</div>
...
<?php
    ...
    if ($query_ubah) {
      ...
      Swal.fire({title: 'Transaksi Berhasil di Proses',text: '',icon: 'success',confirmButtonText: 'OK'
        ...
        {window.location = 'index.php?page=kirim-admin';
        }
      })</script>";
      }else{
      echo "<script>
